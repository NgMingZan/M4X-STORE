(() => {
  ...
  async function generate(){
    ...
    try{const j=await api('generate',{product_id,tone:$('m4xAiTone').value,length:$('m4xAiLength').value,extra_instruction:$('m4xAiExtra').value.trim()});$('m4xAiOutput').value=j.caption||'';$('m4xAiMeta').textContent=j.provider==='openai'?`OpenAI · ${j.model||'model mặc định'}`:'Mẫu dự phòng · chưa có OPENAI_API_KEY';msg(j.warning||'✅ Đã tạo caption. Bạn có thể sửa trước khi đăng.',true,!!j.warning)}catch(e){msg('❌ '+(e?.message||e))}finally{setBusy(false)}
  }
