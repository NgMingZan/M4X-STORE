...
Deno.serve(async(req)=>{
  try{
    ...
    if(!await verifyHmac(raw,ts,sig,secret)) return Response.json({success:false,message:'Invalid signature'},{status:401})
    ...
    if(!expectedAccount||receivedAccount!==expectedAccount) return Response.json({success:true,ignored:'wrong_account'})
    const source=String(p.code||p.content||'').toUpperCase()
    const match=source.match(/M4X[A-F0-9]{10}/)
    if(!match) return Response.json({success:true,ignored:'no_order_code'})
    const transactionId=String(p.id)
    const amount=Number(p.transferAmount||0)
    if(!transactionId||!Number.isFinite(amount)||amount<=0) return Response.json({success:true,ignored:'invalid_payload'})
    const sb=createClient(Deno.env.get('SUPABASE_URL')!,Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!,{auth:{persistSession:false}})
    const {data,error}=await sb.rpc('process_sepay_payment',{p_transaction_id:transactionId,p_order_code:match[0],p_amount:amount,p_content:String(p.content||''),p_account:receivedAccount,p_raw:p})
    if(error) throw error
    return Response.json({success:true,data})
  }catch(e){console.error(e);return Response.json({success:false,message:e.message||'Server error'},{status:500})}
})
