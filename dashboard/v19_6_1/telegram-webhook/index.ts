// M4X Telegram Admin Bot V19.6.1 — callback webhook repair + AI Video
...
function adminMenu(enabled = true) {
  return { inline_keyboard: [
    ...
    [{ text: "🎬 AI Video", callback_data: "adm:video" }],
...
const VIDEO_MODELS: Record<string, { label: string; duration: number; note: string }> = {
...
function videoDurationMenu(model: string) {
  if (model === "wan-fast") return { inline_keyboard: [[{ text: "✅ 5 giây (cố định)", callback_data: "adm:video_duration:5" }], [{ text: "↩️ Menu", callback_data: "adm:home" }]] };
  return { inline_keyboard: [
...
async function generatePollinationsVideo(payload: any) {
  ...
  if (!key) throw new Error("Thiếu POLLINATIONS_API_KEY trong Supabase Edge Secrets.");
  const prompt = clip(payload?.prompt, 900); if (!prompt) throw new Error("Prompt video đang trống.");
  ...
  try {
    ...
    if (!r.ok) {
      let detail = ""; try { detail = clip(await r.text(), 800); } catch (_) {}
      if (/pollen|balance|budget|credit/i.test(detail)) throw new Error(`Không đủ Pollen hoặc key đã chạm giới hạn. ${detail}`.trim());
      throw new Error(`Pollinations lỗi ${r.status}${detail ? `: ${detail}` : ""}`);
    }
    const blob = await r.blob();
    if (!blob.size) throw new Error("Pollinations trả về video rỗng.");
    const maxMb = Math.max(5, Number(env("M4X_VIDEO_MAX_MB", "45")) || 45);
    if (blob.size > maxMb * 1024 * 1024) throw new Error(`Video ${(blob.size / 1024 / 1024).toFixed(1)}MB vượt giới hạn ${maxMb}MB đã đặt cho bot.`);
...
async function startVideoGeneration(chatId: number | string, userId: number | string, payload: any) {
  if (!env("POLLINATIONS_API_KEY")) throw new Error("Chưa cấu hình POLLINATIONS_API_KEY.");
...
async function handleCallback(q: any) {
  ...
  try {
    ...
    if (data === "adm:video") { await answerCallback(q.id); await openVideoPrompt(chatId, userId); return; }
    if (data.startsWith("adm:video_model:")) {
      const model = data.slice("adm:video_model:".length); if (!VIDEO_MODELS[model]) throw new Error("Model video không hợp lệ.");
      const s = await getSession(userId); const payload = { ...(s?.payload || {}), model };
      if (!payload.prompt) throw new Error("Hãy nhập prompt video trước.");
      await setSession(userId, "video_choose_ratio", payload); await answerCallback(q.id);
      await tg("sendMessage", { chat_id: chatId, text: `${videoModelInfo(model).label}\n\nChọn tỷ lệ video:`, reply_markup: videoRatioMenu() }); return;
    }
    if (data.startsWith("adm:video_ratio:")) {
      const code = data.slice("adm:video_ratio:".length); const s = await getSession(userId); const payload = { ...(s?.payload || {}), aspectRatio: ratioFromCode(code) };
      if (!payload.prompt || !payload.model) throw new Error("Phiên tạo video đã hết. Hãy bắt đầu lại.");
      await setSession(userId, "video_choose_duration", payload); await answerCallback(q.id);
      await tg("sendMessage", { chat_id: chatId, text: `📐 ${payload.aspectRatio}\n\nChọn thời lượng:`, reply_markup: videoDurationMenu(payload.model) }); return;
    }
    if (data.startsWith("adm:video_duration:")) {
      const duration = Number(data.slice("adm:video_duration:".length)); const s = await getSession(userId); const payload = { ...(s?.payload || {}), duration: Number.isFinite(duration) ? duration : 5, audio: !!s?.payload?.audio };
      if (!payload.prompt || !payload.model || !payload.aspectRatio) throw new Error("Phiên tạo video đã hết. Hãy bắt đầu lại.");
      await setSession(userId, "video_confirm", payload); await answerCallback(q.id);
      await tg("sendMessage", { chat_id: chatId, text: videoSummary(payload), reply_markup: videoConfirmMarkup(payload.audio) }); return;
    }
    if (data === "adm:video_audio") {
      const s = await getSession(userId); if (!s?.payload?.prompt) throw new Error("Phiên tạo video đã hết.");
      const payload = { ...s.payload, audio: !s.payload.audio }; await setSession(userId, "video_confirm", payload); await answerCallback(q.id, payload.audio ? "Bật âm thanh" : "Tắt âm thanh");
      await tg("sendMessage", { chat_id: chatId, text: videoSummary(payload), reply_markup: videoConfirmMarkup(payload.audio) }); return;
    }
    if (data === "adm:video_generate" || data === "adm:video_repeat") {
      const s = await getSession(userId); const payload = { ...(s?.payload || {}) };
      if (!payload.prompt || !payload.model) throw new Error("Chưa có cấu hình video để tạo.");
      await answerCallback(q.id, "Đang tạo video..."); await startVideoGeneration(chatId, userId, payload); return;
    }
    if (data === "adm:video_publish") {
      const s = await getSession(userId); const p = s?.payload || {}; const fileId = String(p.telegram_file_id || ""); const channel = env("TELEGRAM_CHANNEL_ID");
      if (!channel) throw new Error("Thiếu TELEGRAM_CHANNEL_ID."); if (!fileId) throw new Error("Không tìm thấy file video Telegram để đăng lại.");
      ...
      await audit(userId, "bot_video_publish", { model: p.model, duration: p.duration, aspectRatio: p.aspectRatio }); await answerCallback(q.id, "Đã đăng Channel"); await tg("sendMessage", { chat_id: chatId, text: "✅ Đã đăng AI Video lên Channel." }); return;
    }
...
function webhookEndpoint() {
  ...
  if (!base) throw new Error("Thiếu SUPABASE_URL.");
...
async function repairTelegramWebhook() {
  ...
  const j = await r.json().catch(() => ({}));
  if (!r.ok || !j?.ok) throw new Error(j?.description || `setWebhook lỗi ${r.status}`);
  return j;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (cmd === "/video") {
      if (!admin || chat.type !== "private") return json({ ok: true }); await openVideoPrompt(chat.id, from.id); return json({ ok: true });
    }
    ...
    if (admin && chat.type === "private" && raw) {
      ...
      if (session?.state === "await_video_prompt") {
        ...
        if (prompt.length < 8) { await tg("sendMessage", { chat_id: chat.id, text: "⚠️ Prompt hơi ngắn. Hãy mô tả video rõ hơn một chút." }); return json({ ok: true }); }
        await setSession(from.id, "video_choose_model", { prompt });
        await tg("sendMessage", { chat_id: chat.id, text: `✅ Đã nhận prompt:\n\n${prompt}\n\nChọn model video:`, reply_markup: videoModelMenu() }); return json({ ok: true });
      }
    }
    return json({ ok: true });
  } catch (e) {
