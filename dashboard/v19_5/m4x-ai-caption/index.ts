...
function fallbackCaption(p: any, tone: string, len: string, extra: string) {
  ...
  const lines = [`${icon} ${f.name || "SẢN PHẨM M4X STORE"}`, ""];
...
function buildPrompt(p: any, tone: string, len: string, extra: string) {
  const f = productFacts(p);
...
function outputTextGemini(j: any) {
  const parts: string[] = [];
...
async function generateWithGemini(p: any, tone: string, len: string, extra: string) {
  const key = env("GEMINI_API_KEY") || env("GOOGLE_API_KEY");
  if (!key) return null;
  const model = env("GEMINI_MODEL", "gemini-3.7-flash");
  const maxOutputTokens = len === "long" ? 650 : len === "short" ? 260 : 450;
  const prompt = buildPrompt(p, tone, len, extra);
  const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
    method: "POST",
    headers: { "x-goog-api-key": key, "Content-Type": "application/json" },
    body: JSON.stringify({
      contents: [{ role: "user", parts: [{ text: prompt }] }],
      generationConfig: { maxOutputTokens, temperature: 0.72 },
    }),
  });
  const j = await r.json().catch(() => ({}));
  if (!r.ok) throw new Error(j?.error?.message || `Gemini API lỗi ${r.status}`);
  const caption = outputTextGemini(j);
  if (!caption) throw new Error("Gemini không trả về caption.");
...
async function generateWithOpenAI(p: any, tone: string, len: string, extra: string) {
  ...
  const caption = outputTextOpenAI(j);
  if (!caption) throw new Error("OpenAI không trả về caption.");
  return { caption: caption.slice(0, 3500), model };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "generate") {
      ...
      // 1) Gemini free tier first
      try {
        const gemini = await generateWithGemini(product, tone, len, extra);
        if (gemini) {
          provider = "gemini";
          model = gemini.model;
      ...
      // 2) OpenAI fallback if Gemini is missing, rate-limited, quota-exhausted, or errors
      const openaiFallbackEnabled = !["0", "false", "off", "no"].includes(env("AI_OPENAI_FALLBACK", "true").toLowerCase());
      if (!caption && openaiFallbackEnabled) {
        try {
          const openai = await generateWithOpenAI(product, tone, len, extra);
          if (openai) {
            provider = "openai";
            model = openai.model;
      ...
      // 3) Always-available template fallback
      if (!caption) caption = fallbackCaption(product, tone, len, extra);

      let warning: string | null = null;
      if (provider === "openai" && env("GEMINI_API_KEY")) warning = "Gemini tạm không dùng được nên đã tự chuyển sang OpenAI.";
      if (provider === "template") {
        if (!env("GEMINI_API_KEY") && !env("GOOGLE_API_KEY") && !env("OPENAI_API_KEY")) warning = "Chưa có GEMINI_API_KEY/OPENAI_API_KEY nên đang dùng mẫu dự phòng.";
        else warning = "Các AI provider hiện không dùng được nên đang dùng mẫu dự phòng.";
      }
      ...
      return json({ ok: true, caption, provider, model, warning, provider_errors: providerErrors.slice(0, 2) });
    }
